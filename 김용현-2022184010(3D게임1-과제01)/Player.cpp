...
CAirplanePlayer::CAirplanePlayer()
{
	...
	{
		bullet = std::make_unique<CBulletObject>(m_fBulletEffectiveRange);
...
void CAirplanePlayer::Animate(float fElapsedTime)
{
	...
	for (auto& bullet : m_ppBullets)
	{
		if (bullet->m_bActive) bullet->Animate(fElapsedTime);
	}
}
...
void CAirplanePlayer::Render(HDC hDCFrameBuffer, CCamera *pCamera)
{
	...
	for (auto& bullet : m_ppBullets)
	{
		if (bullet->m_bActive) bullet->Render(hDCFrameBuffer, pCamera);
	}
}
