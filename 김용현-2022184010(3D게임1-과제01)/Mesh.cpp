...
CMesh::CMesh(int nPolygons)
{
	if (nPolygons > 0)
	{
		...
		m_ppPolygons = std::make_unique<std::unique_ptr<CPolygon>[]>(nPolygons);
		m_nReferences = 0;
	}
...
void CMesh::SetPolygon(int nIndex, std::unique_ptr<CPolygon> pPolygon)
{
	...
	{
		m_ppPolygons[nIndex] = std::move(pPolygon);
	}
}
...
CWallMesh::CWallMesh(float fWidth, float fHeight, float fDepth, int nSubRects) : CMesh((4 * nSubRects * nSubRects)+2)
{
	...
	int k = 0;
	std::unique_ptr<CPolygon> pLeftFace;
	for (int i = 0; i < nSubRects; i++)
	{
		for (int j = 0; j < nSubRects; j++)
		{
			pLeftFace = std::make_unique<CPolygon>(4);
			...
			SetPolygon(k++, std::move(pLeftFace));
		}
	}

	std::unique_ptr<CPolygon> pRightFace;
	for (int i = 0; i < nSubRects; i++)
	{
		for (int j = 0; j < nSubRects; j++)
		{
			pRightFace = std::make_unique<CPolygon>(4);
			...
			SetPolygon(k++, std::move(pRightFace));
		}
	}

	std::unique_ptr<CPolygon> pTopFace;
	for (int i = 0; i < nSubRects; i++)
	{
		for (int j = 0; j < nSubRects; j++)
		{
			pTopFace = std::make_unique<CPolygon>(4);
			...
			SetPolygon(k++, std::move(pTopFace));
		}
	}

	std::unique_ptr<CPolygon> pBottomFace;
	for (int i = 0; i < nSubRects; i++)
	{
		for (int j = 0; j < nSubRects; j++)
		{
			pBottomFace = std::make_unique<CPolygon>(4);
			...
			SetPolygon(k++, std::move(pBottomFace));
		}
	}
	...
	pFrontFace->SetVertex(2, CVertex(+fHalfWidth, -fHalfHeight, -fHalfDepth));
	pFrontFace->SetVertex(3, CVertex(-fHalfWidth, -fHalfHeight, -fHalfDepth));
	SetPolygon(k++, std::move(pFrontFace));
	...
	pBackFace->SetVertex(2, CVertex(+fHalfWidth, +fHalfHeight, +fHalfDepth));
	pBackFace->SetVertex(3, CVertex(-fHalfWidth, +fHalfHeight, +fHalfDepth));
	SetPolygon(k++, std::move(pBackFace));
